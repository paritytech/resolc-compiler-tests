...
impl TestConfiguration {
    ...
    pub fn with_config(
        ...
        match key.as_ref() {
            ...
            "revertStrings" => {
                self.revert_strings = Some(value.as_ref().parse()?)
            }
            "allowNonExistingFunctions" => {
                self.allow_non_existing_functions =
                    Some(value.as_ref().parse()?)
            }
            "bytecodeFormat" => {
    ...
    pub fn new_from_pairs(
        ...
        pairs
            .into_iter()
            .try_fold(&mut this, |this, (key, value)| {
                this.with_config(key, value)
            })?;
        Ok(this)
    }
}
