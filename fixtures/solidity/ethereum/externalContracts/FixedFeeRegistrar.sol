...
abstract contract Registrar {
    ...
    function owner(
        string memory _name
    ) public view virtual returns (address o_owner);

    function addr(
        string memory _name
    ) public view virtual returns (address o_address);

    function subRegistrar(
        string memory _name
    ) public view virtual returns (address o_subRegistrar);

    function content(
        string memory _name
    ) public view virtual returns (bytes32 o_content);
}
...
contract FixedFeeRegistrar is Registrar {
    ...
    modifier onlyrecordowner(string memory _name) {
        if (m_record(_name).owner == msg.sender) _;
    }

    function reserve(string memory _name) public payable {
        Record storage rec = m_record(_name);
        if (
            rec.owner == 0x0000000000000000000000000000000000000000 &&
            msg.value >= c_fee
        ) {
    ...
    function addr(string memory _name) public view override returns (address) {
        return m_record(_name).addr;
    }

    function subRegistrar(
        string memory _name
    ) public view override returns (address) {
        return m_record(_name).subRegistrar;
    }

    function content(
        string memory _name
    ) public view override returns (bytes32) {
        return m_record(_name).content;
    }

    function owner(string memory _name) public view override returns (address) {
        return m_record(_name).owner;
    }
    ...
    function m_record(
        string memory _name
    ) internal view returns (Record storage o_record) {
