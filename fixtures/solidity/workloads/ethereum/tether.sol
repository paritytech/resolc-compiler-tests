 ...
 * NOTE: 0.8.x has built-in overflow checks, we add unchecked blocks to preserve the behavior of original contract v0.4.17.
 */
library SafeMath {
...
abstract contract BasicToken is Ownable, ERC20Basic {
    ...
    // original code relies on _totalSupply variable directly, but keep totalSupply() correct
...
contract TetherToken is Pausable, StandardToken, BlackList {
    ...
    function allowance(
        address _owner,
        address _spender
    ) public view override(StandardToken) returns (uint remaining) {
        if (deprecated) {
